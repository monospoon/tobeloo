...
function getConsumer(){
	return JSON.parse(self.data.load("twitter_consumer.json"));
}
...
exports.exec = function(){
	...
	ss.storage.twitter_oauth_temp = getConsumer();
	...
	var message = {
		...
		"parameters": {
			"oauth_consumer_key": ss.storage.twitter_oauth_temp.consumer_key,
			"oauth_signature_method": "HMAC-SHA1"
		}
	};
	
	var accessor = {
		"consumerSecret": ss.storage.twitter_oauth_temp.consumer_secret
	};
	...
	Request({
		...
		"onComplete": function(responce){
			...
			//取得したリクエストトークンを記憶
			ss.storage.twitter_oauth_temp.request_token = token;
			ss.storage.twitter_oauth_temp.request_token_secret = secret;
					...
					if(url_info.host == "www16.atpages.jp" && url_info.path == "/rikuta0209/tobeloo/auth.php"){
						var final_message = {
							...
							"parameters":{
								"oauth_consumer_key": ss.storage.twitter_oauth_temp.consumer_key,
								"oauth_signature_method": "HMAC-SHA1",
								"oauth_token": ss.storage.twitter_oauth_temp.request_token,
						...
						var final_accessor = {
							"consumerSecret": ss.storage.twitter_oauth_temp.consumer_secret,
							"tokenSecret": ss.storage.twitter_oauth_temp.request_token_secret
						};
						...
						var accToken = Request({
							...
							"onComplete": function(responce){
								...
								ss.storage.accounts.twitter.access_token        = params.oauth_token;
								ss.storage.accounts.twitter.access_token_secret = params.oauth_token_secret;
								
								delete ss.storage.twitter_oauth_temp; //一時記憶として利用したsimple-storageの値を削除
								tab.close(); //タブを閉じる
