...
var profile_dir = Cc["@mozilla.org/file/directory_service;1"].getService(Ci.nsIProperties).get("ProfD", Ci.nsIFile);
...
exports.loadPlugin = function(){
	var plugin_dir_name = "tobeloo";
	
	var plugin_dir = profile_dir.clone(plugin_dir_name);
	if(!plugin_dir.exists() || !plugin_dir.isDirectory()){
		plugin_dir.create(Components.interfaces.nsIFile.DIRECTORY_TYPE, 0664);
	}
	
	loadSubScripts(getScriptFiles(plugin_dir), Tobeloo);
}